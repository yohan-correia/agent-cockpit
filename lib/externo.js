...
/**
 * O texto é SÓ um envelope de sistema (`<task-notification>…</task-notification>`) — máquina
 * avisando máquina, nunca fala do dono?
 *
 * ESTRUTURAL, não uma lista de nomes: a mesma tag que hoje é `task-notification` amanhã tem um
 * irmão, e lista fixa é a #40 esperando acontecer (foi o erro que quase se repetiu no conserto
 * do eco de comando). A regra é "abre com uma tag e fecha com a MESMA no fim" — uma fala que
 * apenas CITA a tag no meio de uma frase continua sendo fala.
 */
function ehEnvelopeDeSistema(texto) {
  const abre = /^<([a-z][a-z0-9-]*)>/.exec(texto);
  return Boolean(abre) && texto.endsWith(`</${abre[1]}>`);
}

/**
 * A linha é o registro de uma mensagem que o dono escreveu ENQUANTO o turno rodava, e que o
 * turno absorveu sem ela virar `type: "user"` nenhum?
 *
 * O CLI grava três linhas para essa mensagem — `queue-operation/enqueue`, esta, e um
 * `attachment` com `queued_command` — e o texto aparece nas três. Só ESTA serve:
 *
 * - o `enqueue` também acontece na mensagem que vira turno próprio depois (818 casos medidos
 *   em 728 arquivos do disco), e essa volta como `user`: emitir aqui desenharia a fala 2x;
 * - o `attachment` é a segunda cópia da MESMA mensagem, e não sabe dizer qual das duas é;
 * - `remove` sem `reason` (208 casos) é outra coisa, e no disco inteiro só carregou máquina.
 *
 * `absorbed_mid_turn` é a única que afirma "esta mensagem não volta de outro jeito" — 325 dos
 * 327 casos do disco não têm `user` nenhum com aquele texto.
 */
function ehAbsorvidaNoTurno(obj) {
  if (obj.type !== 'queue-operation' || obj.operation !== 'remove' || obj.reason !== 'absorbed_mid_turn') return false;
  const texto = String(obj.content || '').trim();
  return Boolean(texto) && !ehEnvelopeDeSistema(texto);
}

function textoDoHumano(obj) {
  // O texto já vem cru aqui: é o que o dono digitou, não o eco embrulhado que o CLI grava
  // depois. Não há envelope para desfazer.
  if (ehAbsorvidaNoTurno(obj)) return String(obj.content).trim();
...
function ehFalaDoHumano(obj) {
  // Mensagem escrita com o agente trabalhando: não é `user` nenhum no disco, e sem este ramo
  // ela nunca voltava — nem para a fita, nem para `consumirPendente`. Aqui e não num ramo
  // próprio de `eventosDoObjeto` de propósito: `trocasDeFora` (:254) usa a MESMA função para
  // decidir o que é fala de fora, e duas regras para "o que é fala" seria a #40.
  if (ehAbsorvidaNoTurno(obj)) return true;
